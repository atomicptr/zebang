let parse_line line =
  if String.starts_with line ~prefix:"#!" then
    ...
    match parts with interpreter :: args -> Ok (interpreter :: args) | _ -> Error "Malformed shebang"
...
let parse_file path =
  ...
  try
    ...
    match parse_line first_line with Ok args -> Ok args | Error msg -> Error (msg ^ " in file: " ^ path)
  with
